...
export default async function CampaignDetailPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<SearchParams> }) {
  ...
  return <main className="min-h-screen bg-[#f6f8fb] px-5 py-8 text-slate-950 sm:px-8"><div className="mx-auto max-w-3xl">
    ...
    <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-10">
      ...
      <form action={assignRecipient.bind(null, campaign.id)} className="mt-6 grid gap-3 rounded-xl bg-slate-50 p-4"><select className="rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm" defaultValue="" name="recipientId"><option disabled value="">Select a recipient</option>{recipients.map((recipient) => <option key={recipient.id} value={recipient.id}>{recipient.email}</option>)}</select><label><span className="text-xs font-semibold text-slate-700">Private context for this recipient in this campaign <span className="font-normal text-slate-400">(optional)</span></span><textarea className="mt-2 min-h-24 w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-100" maxLength={2000} name="notes" placeholder="Example: Interested in a two-bedroom unit, prefers Saturday viewings, and asked about flexible payments." /></label><button className="w-fit rounded-xl bg-slate-950 px-4 py-3 text-sm font-semibold text-white" type="submit">Assign recipient</button></form>
      <div className="mt-6 space-y-3">{assignments.map((assignment) => { const link = assignment.links[0]; const assignmentNotes = assignment.context && typeof assignment.context === "object" && "notes" in assignment.context && typeof assignment.context.notes === "string" ? assignment.context.notes : null; return <div className="rounded-xl border border-slate-100 p-4" key={assignment.id}><p className="font-semibold">{assignment.recipient.email}</p>{assignmentNotes && <p className="mt-2 text-xs leading-5 text-slate-500">Private campaign context: {assignmentNotes}</p>}{link ? <div className="mt-4 flex gap-3"><a className="text-xs font-semibold text-indigo-700" href={`/r/${link.token}`} target="_blank">Open link</a><form action={sendCampaignEmail.bind(null, campaign.id, assignment.id)}><button className="text-xs font-semibold text-emerald-700" type="submit">Send email</button></form><form action={revokeRecipientLink.bind(null, campaign.id, link.id)}><button className="text-xs font-semibold text-rose-700" type="submit">Revoke</button></form></div> : <form action={createRecipientLink.bind(null, campaign.id, assignment.id)} className="mt-4"><button className="rounded-lg bg-indigo-50 px-3 py-2 text-xs font-semibold text-indigo-700" type="submit">Generate secure link</button></form>}</div>; })}</div>
    </section>
  </div></main>;
}
